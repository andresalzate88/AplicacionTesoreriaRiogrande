...
## Distribuciones Riogrande · Versión 7.2
**Fecha:** 2026-04-28
...
|--------|-------------------|
| Alpina | Manual por auxiliar — conciliación mensual (~día 15 del mes siguiente) |
...
Vista 05: Sync Odoo
  ...
  Bloque 3: Sync maestro proveedores (manual o automático)
  Bloque 4: Alerta empleados sin Odoo (solo informativa)
  Bloque 5: Sync transacciones (retenciones, gastos, consignaciones, anticipos, traslados)
```
...
| `cuenta_caja` | text | Cuenta PUC de efectivo en caja (130501/02/03/04) — se mantiene por precaución |
...
**`aliados`** — 5 registros. Tabla nueva en v7. El Admin puede editar NIT y razón social.
...
**`operaciones`** — 10 registros. El Admin puede editar. `aliado_id` reemplaza el campo `aliado` texto de versiones anteriores.
...
| `aliado_id` | uuid FK → aliados | Reemplaza campo `aliado` texto |
| `tipo_erp` | text | SIDIS / ECOM |
| `activa` | boolean | Default true |

**`bancos`** — Catálogo con ID numérico (se usa en Excel de ingesta). `tipo`: RIOGRANDE / EXTERNO. Sin cambios desde v6. El UI de consignaciones RG usa `parametros_contables`, no esta tabla directamente.
...
**`cuentas_analiticas`** — Distribuciones analíticas en formato JSON para Odoo. Se eliminó `operacion_id` en v7 — la distribución ya está implícita en `codigo_odoo`. El sistema calcula las combinadas automáticamente dividiendo en partes iguales según las operaciones del cuadre. Ejemplo: cuadre DA+DC → `{"7":50,"8":50}`.
...
**`parametros_contables`** — Rediseñado en v7. 9 tipos fijos. Los tipos NO se pueden crear ni eliminar desde la UI. Solo se pueden agregar más `detalle_asiento` dentro de cada tipo existente.

| Campo | Tipo | Descripción |
|-------|------|-------------|
| `id` | uuid PK | — |
| `sede_id` | uuid FK → sedes nullable | NULL = todas las sedes. UUID = solo esa sede. En UI muestra nombre de sede. |
| `tipo_asiento` | text | 11 tipos fijos (ver guía abajo) |
...
| `banco_id` | integer FK → bancos nullable | Solo consignaciones a riogrande — vincula parámetro con extracto bancario. |
...
**`parametros_contables_generales`** — Tabla nueva en v7. Cuentas contables transversales al sistema no ligadas a un tipo de asiento específico.
...
| `sincronizado_odoo` | boolean | Default false. Alerta en Vista 05 si false y tiene anticipos. |
| `activo` | boolean | Default true |
| `updated_at` | timestamptz | — |
...
**`documentos_erp`** — Facturas, notas crédito/débito y anticipos de clientes. Clave de upsert: `operacion_documento`. El campo `documento_electronico` es la clave de cruce con `documentos_dian`.
...
**Anticipos de clientes:** el plano ERP genera dos tipos de documento para el manejo de anticipos:
...
**`consignaciones_banco`** — Extractos bancarios de Riogrande. El `banco_id` lo pone contabilidad en el Excel. El UI de la sección 2.3 usa `parametros_contables`, no la tabla `bancos` directamente.
...
**`consignaciones_aliados`** — Consignaciones de aliados. Cárnicos/Nutresa/Meals vienen del archivo certificado (estado inicial: CERTIFICADA). Alpina las registra manualmente la auxiliar (estado inicial: SIN_CERTIFICAR).
...
| `sede_id` | uuid FK → sedes | — |
| `operacion_id` | uuid FK → operaciones | Nullable |
| `aliado` | text | ALPINA / CARNICOS / FAMILIA / NUTRESA / MEALS — texto para compatibilidad con ingesta |
...
```
Estado BORRADOR: permite guardar y retomar sin cerrar (útil cuando la consignación del día aún no aparece en el extracto).

**`cuadres`** — Un cuadre agrupa 1 o varias planillas de la misma sede.
...
| `sede_id` | uuid FK → sedes | — |
| `recaudo_id` | uuid FK → recaudos_dia | Nullable — se asigna al pasar al recaudo |
| `fecha_cuadre` | date | — |
| `planillas` | text[] | ['DA-32926', 'DA-32937'] |
| `operaciones` | text[] | ['DA', 'DC'] |
| `estado` | text | BORRADOR / ENVIADO_REVISION / APROBADO / ANULADO |
| `conductor_id` | uuid FK → empleados | — |
| `auxiliar_1_id` | uuid FK → empleados | Nullable |
| `auxiliar_2_id` | uuid FK → empleados | Nullable |
| `vehiculo_id` | uuid FK → vehiculos | — |
| `total_cuadrar_contado` | numeric | Suma facturas contado netas |
| `total_anticipos_clientes` | numeric | Puede ser negativo |
| `total_gastos` | numeric | — |
| `total_consig_riogrande` | numeric | — |
| `total_consig_aliados` | numeric | — |
| `total_anticipos_nomina` | numeric | — |
| `efectivo_real` | numeric | Conteo físico de la auxiliar |
| `efectivo_teorico` | numeric | Calculado por sistema |
| `diferencia_efectivo` | numeric GENERATED | efectivo_real - efectivo_teorico |
...
**`cuadre_retenciones`** — Retenciones de clientes por factura. Solo facturas CONTADO.
...
| `documento_id` | uuid FK → documentos_erp | — |
| `cliente_id` | uuid FK → clientes | Nullable |
...
| `base_retencion` | numeric | Valor base sobre el cual se calcula la retención |
...
**`gastos`** — Gastos de ruta. Cubre secciones 2.2 (PLANILLA) y 3.2 (DESTINO_EFECTIVO). Valor impuesto y valor retención los digita manualmente el usuario — no hay auto-cálculo por porcentaje.
...
| `valor_impuesto` | numeric | Ingresado manualmente por auxiliar. Default 0. |
| `valor_retencion` | numeric | Ingresado manualmente por auxiliar. Default 0. |
...
| `promover_a_historico(recaudo_id)` | Copia 15 tablas al histórico con columnas explícitas. En hist_documentos_erp resuelve fecha_recaudo via join a recaudos_dia. |

**Nota v7:** la función `generar_asientos()` fue eliminada. Cada tabla histórica ya tiene todos los campos necesarios para enviar directamente a Odoo via API sin transformaciones intermedias.
...
**5 bloques — los primeros 3 deben completarse antes del 5:**
...
`proveedores` WHERE `sincronizado_odoo = false` OR `requiere_sync_odoo = true`. Corre en paralelo con bloque 1.

**Bloque 4 — Empleados sin Odoo (solo alerta)**
`hist_empleados` WHERE `sincronizado_odoo = false`. Solo informativo — la creación en Odoo es manual desde nómina. NO bloquea el bloque 5.
...
Muestra todas las transacciones históricas con `estado_odoo = PENDIENTE / ERROR` agrupadas por tipo. Filtros: Sede, Rango de fechas, Tipo, Estado Odoo. Badge de estado: PENDIENTE (gris) / ENVIADO (azul) / CONFIRMADO (verde) / ERROR (rojo).
...
| Elemento | Cambio |
|----------|--------|
| **Arquitectura Odoo** | Eliminada tabla `asientos_contables` y función `generar_asientos()`. Cada histórica sale directo a Odoo via API. |
| **`sedes`** | + `diario_caja` |
| **`aliados`** | Tabla nueva |
| **`operaciones`** | `aliado` text → `aliado_id` FK. Eliminado `letra_aliado`. |
| **`cuentas_analiticas`** | Eliminado `operacion_id` |
| **`parametros_contables`** | Rediseño completo — campos débito/crédito eliminados, + `id_externo_odoo`, `diario_odoo`, `sede_id`, `banco_id` |
| **`parametros_contables_generales`** | Tabla nueva |
...
| **`gastos`** | Renombres + `fecha` + `valor_total` GENERATED + 11 campos desnormalizados. Valor impuesto y retención siempre manuales. |
| **`cuadre_anticipos_nomina`** | + `fecha` + 5 campos desnormalizados |
| **`destinos_efectivo`** | Renombrada a `traslados_caja` + 4 campos nuevos |
| **Históricas** | 15 tablas (antes 14): + `hist_traslados_caja`. Las que viajan a Odoo: + `estado_odoo`, `referencia_odoo`, `error_sync`. |
| **Vista 05** | 5 bloques: + alerta empleados, sync por transacciones directas |

### v7.1 → v7.2
| Elemento | Cambio |
|----------|--------|
| **`documentos_erp.tipo_documento`** | Valores de anticipo separados: `ANTICIPO` (dinero recibido, siempre positivo) y `CRUCE ANTICIPO` (cruce con factura, siempre positivo). Antes: `ANTICIPO` positivo o negativo según el caso. |
| **`documentos_erp.sub_tipo`** | Mismo cambio: valores `ANTICIPO` y `CRUCE ANTICIPO`. |
| **`parametros_contables`** | Eliminados tipos `anticipo recibido clientes` y `descuento anticipo clientes` — la cuenta contable viene del plano ERP, no requiere parametrización. Quedan 9 tipos fijos. |
| **Vista 02 — Sección 2.5** | UI muestra ambos tipos diferenciados: ANTICIPO suma al total, CRUCE ANTICIPO resta. |

---

*Documento interno — Distribuciones Riogrande · Plataforma Tesorería v7.2 · 2026-04-28*
